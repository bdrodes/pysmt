...
class CVC4Options(SolverOptions):
    ...
    def _set_option(cvc4, name, value):
        try:
            cvc4.setOption(name, value)
        except:
            raise PysmtValueError("Error setting the option '%s=%s'" % (name,value))

    def __call__(self, solver):
        if solver.logic_name == "QF_SLIA" or solver.logic_name == "QF_ASLIA" or solver.logic_name == "QF_ASLIRA":
...
class CVC4Solver(Solver, SmtLibBasicSolver, SmtLibIgnoreMixin):
    ...
    def __init__(self, environment, logic, **options):
        ...
        self.options(self)
        self.converter = CVC4Converter(environment, self.cvc4)
        return

    def reset_assertions(self):
        self.cvc4.resetAssertions()

        # del self.cvc4
        # # CVC4's SWIG interface is not acquiring ownership of the
        # # SmtEngine object. Forcing it here.
        # self.cvc4 = pycvc4.Solver() #CVC4.SmtEngine(self.em); self.cvc4.thisown=1
        # self.options(self)
    ...
    def solve(self, assumptions=None):
        if assumptions is not None:
            ...
            res = self.cvc4.checkSatAssuming(cvc4_assumption)
        else:
            try:
    ...
    def set_option(self, name, value):
        ...
        :type value: String
        """
        self.cvc4.setOption(name, value)



class CVC4Converter(Converter, DagWalker):

    def __init__(self, environment, solver:pycvc4.Solver):
        DagWalker.__init__(self, environment)

        self.mkExpr = solver.mkTerm # cvc4_exprMgr.mkExpr
        self.mkConst = solver.mkConst #cvc4_exprMgr.mkConst
        self.mkVar = solver.mkVar #cvc4_exprMgr.mkVar
        self.mkString = solver.mkString 
        self.mkReal = solver.mkReal 
        self.mkBool = solver.mkBoolean
        self.realType = solver.getRealSort() # cvc4_exprMgr.realType()
        self.intType = solver.getIntegerSort() # cvc4_exprMgr.integerType()
        self.boolType = solver.getBooleanSort() # cvc4_exprMgr.booleanType()
        self.stringType = solver.getStringSort() # cvc4_exprMgr.stringType()
    ...
    def declare_variable(self, var):
        ...
        if var.symbol_name() not in self.declared_vars:
            ...
            decl = self.mkVar(cvc4_type, var.symbol_name())
            self.declared_vars[var] = decl

    def back(self, expr):
        res = None
        if expr.isConst():
            if expr.getSort() == self.boolType:
                v = expr.getConstBoolean()
                res = self.mgr.Bool(v)
            elif expr.getSort() == self.intType:
                # v = expr.getConstRational().toString()
                res = self.mgr.Int(int(str(expr)))
            elif expr.getSort() == self.realType:
                # v = expr.getConstRational().toString()
                res = self.mgr.Real(Fraction(str(expr)))
            # elif expr.getType().isBitVector():
            #     bv = expr.getConstBitVector()
            #     v = bv.getValue().toString()
            #     width = bv.getSize()
            #     res = self.mgr.BV(int(v), width)
            elif expr.getSort() == self.stringType:
                # v = expr.getConstString()
                res = self.mgr.String(eval(str(expr)))
    ...
    def walk_and(self, formula, args, **kwargs):
        return self.mkExpr(pycvc4.kinds.And, args)

    def walk_or(self, formula, args, **kwargs):
        return self.mkExpr(pycvc4.kinds.Or, args)

    def walk_not(self, formula, args, **kwargs):
        return self.mkExpr(pycvc4.kinds.Not, args[0])
    ...
    def walk_iff(self, formula, args, **kwargs):
        return self.mkExpr(pycvc4.kinds.Equal, args[0], args[1])

    def walk_implies(self, formula, args, **kwargs):
        return self.mkExpr(pycvc4.kinds.Implies, args[0], args[1])

    def walk_le(self, formula, args, **kwargs):
        return self.mkExpr(pycvc4.kinds.Leq, args[0], args[1])

    def walk_lt(self, formula, args, **kwargs):
        return self.mkExpr(pycvc4.kinds.Lt, args[0], args[1])

    def walk_ite(self, formula, args, **kwargs):
        return self.mkExpr(pycvc4.kinds.Ite, args[0], args[1], args[2])

    def walk_real_constant(self, formula, **kwargs):
        frac = formula.constant_value()
        n,d = frac.numerator, frac.denominator
        rep = str(n) + "/" + str(d)
        return self.mkReal(rep)
    ...
    def walk_bool_constant(self, formula, **kwargs):
        return self.mkBool(formula.constant_value())
    ...
    def walk_exists(self, formula, args, **kwargs):
        ...
        bound_vars_list = self.mkExpr(pycvc4.kinds.BoundVarList, var_list)
        return self.mkExpr(pycvc4.kinds.Exists,
                           bound_vars_list,
                           bound_formula)
    ...
    def walk_forall(self, formula, args, **kwargs):
        (bound_formula, var_list) = \
                 self._rename_bound_variables(args[0], formula.quantifier_vars())
        bound_vars_list = self.mkExpr(pycvc4.kinds.BoundVarList, var_list)
        return self.mkExpr(pycvc4.kinds.ForAll,
                           bound_vars_list,
                           bound_formula)

    def walk_plus(self, formula, args, **kwargs):
        res = self.mkExpr(pycvc4.kinds.Plus, args)
        return res

    def walk_array_store(self, formula, args, **kwargs):
        return self.mkExpr(pycvc4.kinds.Store, args[0], args[1], args[2])

    def walk_array_select(self, formula, args, **kwargs):
        return self.mkExpr(pycvc4.kinds.Select, args[0], args[1])

    def walk_minus(self, formula, args, **kwargs):
        return self.mkExpr(pycvc4.kinds.Minus, args[0], args[1])
    ...
    def walk_times(self, formula, args, **kwargs):
        ...
        res = args[0]
        for x in args[1:]:
            res = self.mkExpr(pycvc4.kinds.Mult, res, x)
        return res

    def walk_toreal(self, formula, args, **kwargs):
        return self.mkExpr(pycvc4.kinds.ToReal, args[0])
    ...
    def walk_function(self, formula, args, **kwargs):
        ...
        return self.mkExpr(pycvc4.kinds.ApplyUf, decl, args)
    ...
    def walk_bv_ult(self, formula, args, **kwargs):
        return self.mkExpr(pycvc4.kinds.BVUlt, args[0], args[1])

    def walk_bv_ule(self, formula, args, **kwargs):
        return self.mkExpr(pycvc4.kinds.BVUle, args[0], args[1])

    def walk_bv_concat(self, formula, args, **kwargs):
        return self.mkExpr(pycvc4.kinds.BVConcat, args[0], args[1])

    def walk_bv_extract(self, formula, args, **kwargs):
        ext = self.mkConst(CVC4.BitVectorExtract(formula.bv_extract_end(),
                                                 formula.bv_extract_start()))
        return self.mkExpr(pycvc4.kinds.BVExtract, ext, args[0])

    def walk_bv_or(self, formula, args, **kwargs):
        return self.mkExpr(pycvc4.kinds.BVOr, args[0], args[1])

    def walk_bv_not(self, formula, args, **kwargs):
        return self.mkExpr(pycvc4.kinds.BVNot, args[0])

    def walk_bv_and(self, formula, args, **kwargs):
        return self.mkExpr(pycvc4.kinds.BVAnd, args[0], args[1])

    def walk_bv_xor(self, formula, args, **kwargs):
        return self.mkExpr(pycvc4.kinds.BVXor, args[0], args[1])

    def walk_bv_add(self, formula, args, **kwargs):
        return self.mkExpr(pycvc4.kinds.BVPlus, args[0], args[1])

    def walk_bv_sub(self, formula, args, **kwargs):
        return self.mkExpr(pycvc4.kinds.BVSub, args[0], args[1])

    def walk_bv_neg(self, formula, args, **kwargs):
        return self.mkExpr(pycvc4.kinds.BVNeg, args[0])

    def walk_bv_mul(self, formula, args, **kwargs):
        return self.mkExpr(pycvc4.kinds.BVMult, args[0], args[1])

    def walk_bv_tonatural(self, formula, args, **kwargs):
        return self.mkExpr(pycvc4.kinds.BVToNat, args[0])
    ...
    def walk_bv_udiv(self, formula, args, **kwargs):
        ...
        if d.isConst():
            ...
            v = bv.getValue().toString()
            if v == "0":
                return self.mkExpr(pycvc4.kinds.BVNot, d)
            else:
                return self.mkExpr(pycvc4.kinds.BVUdiv, n, d)
        else:
            # (d == 0) ? ~0 : n bvudiv d
            base = self.mkExpr(pycvc4.kinds.BVUdiv, n, d)
            zero = self.mkConst(CVC4.BitVector(formula.bv_width(),
                                               CVC4.Integer("0")))
            notzero = self.mkExpr(pycvc4.kinds.BVNot, zero)
            test = self.mkExpr(pycvc4.kinds.Equal, d, zero)
            return self.mkExpr(pycvc4.kinds.Ite, test, notzero, base)
    ...
    def walk_bv_urem(self, formula, args, **kwargs):
        ...
        if d.isConst():
            ...
            if v == "0":
                ...
                return self.mkExpr(pycvc4.kinds.BVUrem, n, d)
        else:
            # (d == 0) ? n : n bvurem d
            base = self.mkExpr(pycvc4.kinds.BVUrem, n, d)
            zero = self.mkConst(CVC4.BitVector(formula.bv_width(),
                                               CVC4.Integer("0")))
            test = self.mkExpr(pycvc4.kinds.Equal, d, zero)
            return self.mkExpr(pycvc4.kinds.Ite, test, n, base)

    def walk_bv_lshl(self, formula, args, **kwargs):
        return self.mkExpr(pycvc4.kinds.BVShl, args[0], args[1])

    def walk_bv_lshr(self, formula, args, **kwargs):
        return self.mkExpr(pycvc4.kinds.BVLshr, args[0], args[1])

    def walk_bv_rol(self, formula, args, **kwargs):
        ext = self.mkConst(CVC4.BitVectorRotateLeft(formula.bv_rotation_step()))
        return self.mkExpr(pycvc4.kinds.BVRotateLeft, ext, args[0])

    def walk_bv_ror(self, formula, args, **kwargs):
        ext = self.mkConst(CVC4.BitVectorRotateRight(formula.bv_rotation_step()))
        return self.mkExpr(pycvc4.kinds.BVRotateRight, ext, args[0])
    ...
    def walk_bv_sext (self, formula, args, **kwargs):
        ...
        return self.mkExpr(pycvc4.kinds.BVSignExtend, ext, args[0])

    def walk_bv_slt(self, formula, args, **kwargs):
        return self.mkExpr(pycvc4.kinds.BVSlt, args[0], args[1])

    def walk_bv_sle(self, formula, args, **kwargs):
        return self.mkExpr(pycvc4.kinds.BVSle, args[0], args[1])

    def walk_bv_comp(self, formula, args, **kwargs):
        return self.mkExpr(pycvc4.kinds.BVComp, args[0], args[1])
    ...
    def walk_str_length(self, formula, args, **kwargs):
        return self.mkExpr(pycvc4.kinds.StringLength , args[0])

    def walk_str_concat(self, formula, args, **kwargs):
        return self.mkExpr(pycvc4.kinds.StringConcat, args)

    def walk_str_contains(self, formula, args, **kwargs):
        return self.mkExpr(pycvc4.kinds.StringContains, args[0], args[1])

    def walk_str_indexof(self, formula, args, **kwargs):
        return self.mkExpr(pycvc4.kinds.StringIndexof, args[0], args[1], args[2])

    def walk_str_replace(self, formula, args, **kwargs):
        return self.mkExpr(pycvc4.kinds.StringReplace, args[0], args[1], args[2])

    def walk_str_substr(self, formula, args, **kwargs):
        return self.mkExpr(pycvc4.kinds.StringSubstr, args[0], args[1], args[2])

    def walk_str_prefixof(self, formula, args, **kwargs):
        return self.mkExpr(pycvc4.kinds.StringPrefix, args[0], args[1])

    def walk_str_suffixof(self, formula, args, **kwargs):
        return self.mkExpr(pycvc4.kinds.StringSuffix, args[0], args[1])

    def walk_str_to_int(self, formula, args, **kwargs):
        return self.mkExpr(pycvc4.kinds.StringToInt, args[0])

    def walk_int_to_str(self, formula, args, **kwargs):
        return self.mkExpr(pycvc4.kinds.StringFromInt, args[0])

    def walk_str_charat(self, formula, args, **kwargs):
        return self.mkExpr(pycvc4.kinds.StringCharat, args[0], args[1])
